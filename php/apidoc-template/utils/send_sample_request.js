define([
  ...
  function sendSampleRequest(group, name, version, type)
  {
      ...
      function displaySuccess(data, status, jqXHR) {
          var jsonResponse, objResponse;
          try {
              objResponse = jsonResponse = JSON.parse(jqXHR.responseText);
              jsonResponse = JSON.stringify(jsonResponse, null, 4);
          } catch (e) {
              jsonResponse = data;
          }
          $root.find(".sample-request-response-json").html(jsonResponse);
          if (globalRequestUrl.indexOf('admin/base/login') > -1 && objResponse.code == 200) {
              var globalHeader = {};
              globalHeader['authkey'] = objResponse.data.authKey;
              globalHeader['sessionid'] = objResponse.data.sessionId;
              Cookies.set('globalHeader', globalHeader);
          }

          if (globalRequestUrl.indexOf('admin/base/logout') > -1) {
              Cookies.set('globalHeader', null);
          }
